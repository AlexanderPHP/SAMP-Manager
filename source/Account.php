...
class Account{
	...
	{	
		...
		{
			$pass = $app['db']->fetchAssoc('SELECT `password` FROM `site_users` WHERE `id` = ?',array($_COOKIE['uid']));

			if(!empty($pass) && $pass[0]['key'] == $_COOKIE['pwd'])
			{
				$app['site_user'] = $app['db']->fetchAssoc('SELECT `id`,`name`,`email`,`group`,`avatar` FROM `site_users` WHERE `id` = ?',array($_COOKIE['uid']));
				$app['db']->update('UPDATE `game_users` SET `last_visit` = CURRENT_TIMESTAMP WHERE `id` = :id',array(':id'=>$_COOKIE['uid']));
	...
	private function isIssetUser($data,$opt)
	{
		switch($opt)
		{
			case 'email': $app['db']->getResult('SELECT `id` FROM `site_users` WHERE `email` = :email',array(':email'=>$data));break;
				
			case 'login': $app['db']->noResult('SELECT `id` FROM `site_users` WHERE `name` = :name',array(':name'=>$data));break;
			
			case 'game': $app['db']->noResult('SELECT `id` FROM `game_users` WHERE `name` = :name',array(':name'=>$data));break;
		}
		if($app['db']->rcount()) return true;
	...
	{
		////////////////////////////////////////// TODO PhoneNr//////////////////////////////////////////// 	
		$app['db']->noResult('INSERT INTO `site_users`(`name`,`key`,`email`) VALUES(:name,:key,:email)',array(':name'=>$name,':key'=>md5($password),':email'=>$email));
		$app['db']->noResult("INSERT INTO `game_users`(`name`,`key`,`phonenr`,`pdatareg`,`pipreg`,`pip`) VALUES(:name,:password,".rand(100000,999999).",'".date('Y-m-d')."','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."')",array(':name'=>$name,':password'=>strtoupper(md5($password))));
	...
	private function ValidateEmailID($name)
	{
		...
		$app['db']->noResult("UPDATE `site_users` SET `activate_code` = '".$code."' WHERE `name` = :name",array(':name'=>$name));
		return $code;
	}
...
TEXT;
    ...
    public function ActivateAccount($uname,$code)
    {
		...
		{
			$ucode = $app['db']->getResult('SELECT `activate_code` FROM `site_users` WHERE `name` = :name',array(':name'=>$uname))[0]['activate_code'];
		  
			if($code == $ucode)
			{
				$app['db']->noResult('UPDATE `site_users` SET `group` = 1 WHERE `name` = :name',array(':name'=>$uname));
	...
	public function Login($name,$password)
	{
		...
		{
			$data = $app['db']->getResult('SELECT `key`,`id` FROM `site_users` WHERE `name` = :name',array(':name'=>$name));
	...
	public function getProfile($user)
	{
		...
		{
			return $app['db']->getResult("SELECT `name`,DATE_FORMAT(`pdatareg`,'%e %M %Y') as `pdatareg`,DATE_FORMAT(`last_visit`,'%e %M %Y') as `last_visit`,`level`,`sex` FROM `game_users` WHERE `name` = :name",array(':name'=>$user))[0];
		}
	}
	
	public function getUsers()
	{
		return $app['db']->getResult('SELECT `name`,`level`,`pip`,`jailed`,`warnings`,`ban`,`muted`,`adminlevel` FROM `game_users`');
	}
		...
		if($this->isIssetUser($uname,'game'))
		{
			switch($type)
			{
				case 'mute': 
							...
							{
								$app['db']->noResult('UPDATE `game_users` SET `muted` = 1 WHERE `name` = :name',array(':name'=>$uname));
								echo 'Пользователь '.$uname.' заткнут.';
							}
							else
							{
								$app['db']->noResult('UPDATE `game_users` SET `muted` = 0 WHERE `name` = :name',array(':name'=>$uname));
				...
				case 'jail':
							if($this->CheckUserAction($type,$uname))
							{ 
								$app['db']->noResult('UPDATE `game_users` SET `jailed` = 1 WHERE `name` = :name',array(':name'=>$uname));
								echo 'Пользователь '.$uname.' посажен в тюрьму.';
							}
							else
							{
								$app['db']->noResult('UPDATE `game_users` SET `jailed` = 0 WHERE `name` = :name',array(':name'=>$uname));
				...
				case 'ban':
							if($this->CheckUserAction($type,$uname))
							{
								$app['db']->noResult('UPDATE `game_users` SET `ban` = 1 WHERE `name` = :name',array(':name'=>$uname));
								echo 'Пользователь '.$uname.' забанен.';
							}
							else
							{
								$app['db']->noResult('UPDATE `game_users` SET `ban` = 0 WHERE `name` = :name',array(':name'=>$uname));
				...
				case 'warn':							
							...
							if($result == 0)
							{
								$app['db']->noResult('UPDATE `game_users` SET `warnings` = 1 WHERE `name` = :name',array(':name'=>$uname));
								echo 'Пользователь '.$uname.' получил предупреждение. Количество предупреждений: 1';
							}
							elseif($result == 1)
							{
								$app['db']->noResult('UPDATE `game_users` SET `warnings` = 2 WHERE `name` = :name',array(':name'=>$uname));
								echo 'Пользователь '.$uname.' получил предупреждение. Количество предупреждений: 2';
							}
							elseif($result == 2)
							{
								$app['db']->noResult('UPDATE `game_users` SET `warnings` = 3 WHERE `name` = :name',array(':name'=>$uname));
				...
				case 'unwarn':							
							...
							elseif($result == 1)
							{
								$app['db']->noResult('UPDATE `game_users` SET `warnings` = 0 WHERE `name` = :name',array(':name'=>$uname));
				...
				case 'delete':
								...
								{
									$app['db']->noResult('DELETE FROM `game_users` WHERE `name` = :name',array(':name'=>$uname));
									$app['db']->noResult('DELETE FROM `site_users` WHERE `name` = :name',array(':name'=>$uname));
	...
	private function CheckUserAction($type,$uname)
	{
		switch($type)
		{
			case 'mute': if($app['db']->getResult('SELECT `muted` FROM `game_users` WHERE `name` = :name',array(':name'=>$uname))[0]['muted'] === '0')return true;break;
			
			case 'jail': if($app['db']->getResult('SELECT `jailed` FROM `game_users` WHERE `name` = :name',array(':name'=>$uname))[0]['jailed'] === '0')return true;break;
			
			case 'ban': if($app['db']->getResult('SELECT `ban` FROM `game_users` WHERE `name` = :name',array(':name'=>$uname))[0]['ban'] === '0')return true;break;		
			
			case 'unwarn':
			case 'warn': return $app['db']->getResult('SELECT `warnings` FROM `game_users` WHERE `name` = :name',array(':name'=>$uname))[0]['warnings'];
		}
